...
import { useLatestPsychologySnapshot, DecisionReadiness, DecisionOverlay, NarrativePersistence, TemporalAttribution, NarrativeOutcome } from "@/hooks/use-psychology-snapshot";
...
function TemporalAttributionBadge({
  attribution
}: {
  ...
  const dominantPeriods = Object.entries(effective_weights).filter(([_, weight]) => weight > 0.25).map(([period]) => period);
...
function LensReadinessCard({
  ...
  return <Card className="p-4 glass-card h-full flex flex-col">
      ...
      <div className={cn("flex-1", !isExpanded && "min-h-[240px] max-h-[240px] overflow-hidden")}>
        ...
        {/* Blocking Narratives */}
        {readiness.blocking_narratives.length > 0 && <div className="mb-3">
            <div className="flex items-center gap-1.5 mb-1.5">
              <TrendingDown className="h-3 w-3 text-bearish" />
              <span className="text-xs font-medium text-bearish">Blocking</span>
            </div>
            <div className="flex flex-wrap gap-1">
              {readiness.blocking_narratives.slice(0, 2).map((narrative, idx) => <Badge key={idx} variant="outline" className="text-[10px] border-bearish/30 text-bearish bg-bearish/5">
                  {narrative}
                </Badge>)}
              {readiness.blocking_narratives.length > 2 && <Badge variant="outline" className="text-[10px]">
        ...
        {readiness.supportive_narratives.length > 0 && <div className="mb-3">
            <div className="flex items-center gap-1.5 mb-1.5">
              <TrendingUp className="h-3 w-3 text-bullish" />
              <span className="text-xs font-medium text-bullish">Supportive</span>
            </div>
            <div className="flex flex-wrap gap-1">
              {readiness.supportive_narratives.slice(0, 2).map((narrative, idx) => <Badge key={idx} variant="outline" className="text-[10px] border-bullish/30 text-bullish bg-bullish/5">
                  {narrative}
                </Badge>)}
              {readiness.supportive_narratives.length > 2 && <Badge variant="outline" className="text-[10px]">
        ...
        {readiness.recommended_delay && readiness.recommended_delay !== "None" && <div className="text-xs text-muted-foreground italic mb-3">
            Recommended delay: {readiness.recommended_delay}
          </div>}
      </div>

      {/* Show Details Toggle Button - Always at bottom */}
      {overlay && <Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="w-full text-xs text-muted-foreground hover:text-foreground pt-2 border-t border-border/30">
          {isExpanded ? <>
      ...
      <AnimatePresence>
        {overlay && isExpanded && <motion.div initial={{
        height: 0,
        opacity: 0
      }} animate={{
        height: "auto",
        ...
        ease: "easeInOut"
            ...
            <div className="pt-3 border-t border-border/50 space-y-3 mt-2">
              ...
              {narrativeOutcomes && narrativeOutcomes.length > 0 && <NarrativeImpactSummary outcomes={narrativeOutcomes} maxDisplay={1} />}
            </div>
          </motion.div>}
      </AnimatePresence>
    </Card>;
...
export function DecisionReadinessDashboard({
  symbol
}: DecisionReadinessDashboardProps) {
  ...
  return <div className="space-y-4">
      ...
      {sortedEntries.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
          {sortedEntries.map(([lensKey, readiness]) => <LensReadinessCard key={lensKey} lensKey={lensKey} readiness={readiness} overlay={interpretation.decision_overlays[lensKey]} narrativeOutcomes={snapshot.narrative_outcomes} />)}
