...
const Index = () => {
  ...
  return (
    <div className={`min-h-screen cursor-light-enabled relative transition-opacity duration-[4000ms] ease-out ${bgLoaded ? 'opacity-100' : 'opacity-0'}`}>
      {/* Theme-aware background images with smooth crossfade animation */}
      <div 
        className={`fixed inset-0 -z-10 bg-cover bg-center bg-no-repeat transition-all duration-[4000ms] ease-out ${bgLoaded ? 'scale-100 blur-0' : 'scale-105 blur-sm'} ${bgLoaded ? 'dark:opacity-0 opacity-100' : 'opacity-0'}`}
        style={{ backgroundImage: bgLoaded ? "var(--landing-bg-light)" : undefined }}
      />
      <div 
        className={`fixed inset-0 -z-10 bg-cover bg-center bg-no-repeat transition-all duration-[4000ms] ease-out ${bgLoaded ? 'scale-100 blur-0' : 'scale-105 blur-sm'} ${bgLoaded ? 'dark:opacity-100 opacity-0' : 'opacity-0'}`}
