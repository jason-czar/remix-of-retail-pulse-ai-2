...
const caseStudies = [{
  ...
  investorResponse: ["Retail sentiment shifted 56% more positive on innovation within 30 days of announcement", "Discussions pivoted from 'stagnant' to 'positioning for AI future'", "StockTwits 'bullish' mentions increased 3x in following quarter"]
}, {
  ...
  investorResponse: ["Stock price increased 34% from announcement to completion", "Retail 'hold' ratings increased from 42% to 71% during transition", "Social sentiment shifted from 'legacy dinosaur' to 'smart pivot'"]
}, {
  ...
  investorResponse: ["68% positive retail sentiment within 48 hours vs. 28% in initial testing", "Discussions praised 'disciplined deal structure' and 'smart valuation'", "Stock held steady through announcement vs. expected 8-12% decline"]
}, {
  ...
  investorResponse: ["Retail sentiment improved 41 points within 60 days of announcement", "StockTwits discussions shifted from 'fire the CEO' to 'finally taking action'", "Stock gained 18% in quarter following leadership change"]
}, {
  ...
  investorResponse: ["Post-earnings sentiment improved 42% vs. previous quarter", "Discussions shifted from 'growth slowing' to 'cash flow machine'", "Stock volatility dropped 28% in week following earnings"]
}, {
  ...
  investorResponse: ["Retail sentiment shifted 52% more positive within 45 days", "Discussions praised 'cutting losses' and 'disciplined capital allocation'", "Stock gained 23% from announcement through execution"]
}];
const CaseStudiesSection = () => {
  const [showAll, setShowAll] = useState(false);
  const displayedStudies = showAll ? caseStudies : caseStudies.slice(0, 3);
  return <section id="intelligence-patterns" className="py-16 md:py-20 px-4 scroll-mt-20">
      <div className="container max-w-6xl mx-auto">
        ...
        <StaggerContainer className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayedStudies.map((study, index) => <StaggerItem key={index}>
              <motion.div className="glass-panel glass-interactive p-6 flex flex-col h-full" whileHover={{
            scale: 1.01
          }} transition={{
            duration: 0.2
          }}>
                ...
                <div className="mt-auto pt-4 border-t border-border/30 dark:border-white/10">
                  ...
                  <ul className="space-y-2">
                    {study.investorResponse.map((response, idx) => <li key={idx} className="flex items-start gap-2 text-sm text-muted-foreground">
        ...
        <ScrollReveal delay={0.2}>
          <div className="flex justify-center mt-10">
            <motion.button onClick={() => setShowAll(!showAll)} className="glass-inset inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-foreground transition-all duration-200 hover:scale-105" whileHover={{
            y: -1
          }} whileTap={{
            scale: 0.98
          }}>
...
const LearnMorePage = () => {
  return <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1">
        ...
        {/* The Core Insight */}
        <section className="glass-section py-16 md:py-20 px-4 bg-background">
          <div className="container max-w-3xl mx-auto">
            <ScrollReveal>
            ...
            <ScrollReveal delay={0.1}>
              <div className="space-y-6 text-muted-foreground leading-relaxed">
                ...
                <div className="grid md:grid-cols-2 gap-6 my-8">
                  <motion.div className="glass-inset p-5" whileHover={{
                  scale: 1.02
                }} transition={{
                  duration: 0.2
                }}>
              ...
              <div className="space-y-6 text-muted-foreground leading-relaxed">
                ...
                <div className="my-8 space-y-4">
                  ...
                  <StaggerContainer staggerDelay={0.08} className="space-y-3">
                    {["M&A skepticism detected weeks before official announcement", "Capital allocation narratives turning negative before earnings", "Leadership credibility erosion visible in conversation patterns", "Competitive repositioning signals emerging from investor discourse"].map((item, idx) => <StaggerItem key={idx}>
                        <motion.div className="glass-list-item p-4 text-sm" whileHover={{
                      x: 4
                    }} transition={{
                      duration: 0.15
                    }}>
              ...
              <div className="glass-panel p-8 md:p-10">
                ...
                <div className="space-y-6 text-muted-foreground leading-relaxed">
                  ...
                  <div className="grid sm:grid-cols-3 gap-4 my-8">
                    {["No analyst bias", "No selective reading", "No lag"].map((item, idx) => <motion.div key={idx} className="glass-inset p-4 text-center" whileHover={{
                    scale: 1.03,
                    y: -2
                  }} transition={{
                    duration: 0.2
          ...
          <div className="container max-w-3xl mx-auto">
            ...
            <StaggerContainer staggerDelay={0.12} className="grid gap-6">
              ...
              description: "Detect perception gaps early. Prepare leadership for the questions investors will ask."
                  ...
                  <motion.div className="glass-panel glass-interactive p-6 flex gap-4" whileHover={{
                scale: 1.01
              }} transition={{
                duration: 0.2
              }}>
            ...
            <ScrollReveal>
              <div className="glass-panel p-8 md:p-10">
                ...
                <ul className="space-y-3 text-muted-foreground mb-8">
                  {["Not social listening software", "Not retail trading signals", "Not hype-driven sentiment dashboards"].map((item, idx) => <motion.li key={idx} className="flex items-center gap-3" initial={{
                  opacity: 0,
                  x: -10
                }} whileInView={{
                  opacity: 1,
